    ...
    internal class CookieUi
    {
        ...
        public void DrawMainWindow()
        {
            ...
            for (var row = 0; row < configuration.Senders.Count; row++)
            {
                ...
                ImGui.SetNextItemWidth(210);
                ImGui.InputText($"##FirstName{row}", ref player.FirstName, CookieHelper.NameLength(player.FamilyName));
                ImGui.TableNextColumn();
                ImGui.SetNextItemWidth(210);
                ...
                ImGui.TableNextColumn();
                if (ImGui.Button($"{FontAwesomeIcon.TrashAlt.ToIconString()}##Delete{row}")) DeletePlayer(row);
                ImGui.PopFont();
                ImGui.Separator();
            }
